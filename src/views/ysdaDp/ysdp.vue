  ...
  <el-card>
    ...
    <el-row style="margin-top: 40px">
      <el-table :data="initFormData" border stripe max-height="750">
        ...
        <el-table-column label="吊牌" prop="diaoPai" />
...
export default {
  ...
  methods: {
    ...
    deleteData(id) {
      this.$confirm('确认删除？')
        .then(_ => {
          var url = '/api/deleteYarnDiaoPai/' + id
          deleteDiaoPai(url).then(res => {
            if (res.status !== 200) {
    ...
    editData(id) {
      var url = '/api/getOneYarnDiaoPai/' + id
      getOneYarnDiaoPai(url).then(res => {
        window.console.log(res.data.data)
